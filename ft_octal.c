...
int			ft_octal(long long int d, t_flags *f)
{
	...
	f->mod && !ft_strcmp(f->mod, "h") && f->tp != 'O' ? d = (unsigned short)d : 0;
	f->mod && !ft_strcmp(f->mod, "hh") && f->tp != 'O' ? d = (unsigned char)d : 0;
	...
	(f->tchn_t == 1 && f->tchn > i) ? tmp = ft_strtemp(f->tchn, i, '0') : 0;
	(f->tchn_t == 1 && f->tchn > i) ? s = ft_strjoinfree(tmp, s, 3) : 0;
	f->resh == 1 && !f->zr && f->tchn_t < i && d ? s = ft_strjoinfree("0", s, 2) : 0;
	(f->min == 1 && f->w < 0) ? f->w = -f->w : 0;
...
int			ft_unsig(long long int d, t_flags *f, int i)
{
	char	*s;
...
int			ft_hex(long long int d, t_flags *f)
{
	...
	return (ft_printnbr(f->tp == 'X' ? s : ft_strlowcase(s)));
}
