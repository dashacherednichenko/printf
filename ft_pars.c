...
//	printf("DDD%d\n", x);
	while (s[i])
	{
		...
		s[i] == '%' ? i++ : i;
//		if (ft_valid_type(s , i) == 0)
//			return (ft_print_txt(s, f, i));
		while (!ft_strrchr("cspdDiouUxXf%", s[i]) && s[i])
		{
			s[i] == '-' ? f->min = 1 : 0;
			s[i] == ' ' ? f->space = 1 : 0;
			s[i] == '+' ? f->plus = 1 : 0;
			...
			if (s[i] == 'l')
				f->mod = s[++i] == 'l' ? "ll" : "l";
			s[i] == 'L' ? f->mod = "L" : 0;
 // 			i = (ft_strrchr("cspdDiouUxXf%", s[i]) && s[i]) ? i + 1 : ft_print_txt(s, f, i);
			(!ft_strrchr("cspdDiouUxXf%", s[i]) && s[i]) ? i++ : 0;
		}
		if (s[i])
			ft_strrchr("cspdDiouUxXf%", s[i]) ? f->l = ft_pars_type(ar, f, i++, s) : 0;
		s[i] != '%' && s[i] ? i = ft_print_txt(s, f, i) : 0;
	}
	return (f->l);
}
